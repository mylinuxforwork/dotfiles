...
if [ -f $wallpapereffect ]; then
    ...
    if [ ! "$effect" == "off" ]; then
        ...
        if [ -f $generatedversions/$effect-$wallpaperfilename ] && [ "$force_generate" == "0" ] && [ "$use_cache" == "1" ]; then
            ...
            dunstify "Using wallpaper effect $effect..." "with image $wallpaperfilename" -h int:value:10 -h string:x-dunst-stack-tag:wallpaper
...
if [ -f $generatedversions/blur-$blur-$effect-$wallpaperfilename.png ] && [ "$force_generate" == "0" ] && [ "$use_cache" == "1" ]; then
    ...
    dunstify "Generate new cached wallpaper blur-$blur-$effect-$wallpaperfilename with blur $blur" -h int:value:50 -h string:x-dunst-stack-tag:wallpaper
