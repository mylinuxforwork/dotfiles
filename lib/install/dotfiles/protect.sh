...
if [ -d ~/$dot_folder/.config ]; then 
    ...
    _writeMessage "The script will check for file PROTECTED in subfolders of .config"
    _writeMessage "These folders and their files are not overwritten."
    echo 
    files=$(ls -a ~/$dot_folder/.config)
    for f in $files; do
        if [ ! "$f" == "." ] && [ ! "$f" == ".." ]; then
            if [ -d  ~/$dot_folder/.config/$f ] ;then
                _writeLogTerminal 0 "Checking for directory ~/.config/$f"
                if [ -f $HOME/.config/$f/PROTECTED ]; then
                    if [ -d $ml4w_directory/$version/.config/$f ]; then
                        _writeLogTerminal 0 "Folder ~/.config/$f is protected"
